...
src_prepare() {
	epatch "${FILESDIR}"/${PN}-2.05.1-build_system.patch
...
src_install() {
	...
	dolib.so lib${PN}-${PV}.so.0.0.0
	dosym lib${PN}-${PV}.so.0.0.0 /usr/$(get_libdir)/lib${PN}.so.0
	dosym lib${PN}-${PV}.so.0.0.0 /usr/$(get_libdir)/lib${PN}.so
	if use java ; then
		dolib.so lib${PN}j-${PV}.so.0.0.0
		dosym lib${PN}j-${PV}.so.0.0.0 /usr/$(get_libdir)/lib${PN}j.so.0
		dosym lib${PN}j-${PV}.so.0.0.0 /usr/$(get_libdir)/lib${PN}j.so
	fi
